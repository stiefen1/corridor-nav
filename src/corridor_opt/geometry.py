...
class GeometryWrapper:
    def __init__(self, xy: list | None = None, polygon: Geometry | None = None, geometry_type: type = Geometry):
        if polygon is None and xy is not None:
            self._geometry: Geometry = geometry_type(xy)
            self._geometry_type = geometry_type
        elif polygon is not None:
            self._geometry: Geometry = polygon
    ...
    def rotate(self, angle: float, origin=None, **kwargs) -> "GeometryWrapper":
        ...
        new = deepcopy(self)
        new._geometry = affinity.rotate(self._geometry, angle, origin=origin, **kwargs)
        return new
...
def get_geometry_from_object(geometry: Geometry | GeometryWrapper | Tuple) -> Any:
    if isinstance(geometry, (Polygon, LineString)):
        ...
        return geometry
    elif isinstance(geometry, GeometryWrapper):
        return geometry()
    else:
        raise TypeError(f"geometry must be a Geometry, GeometryWrapper or Tuple, not {type(geometry)}")